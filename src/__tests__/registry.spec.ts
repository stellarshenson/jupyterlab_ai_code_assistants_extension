...
describe('descriptor filtering', () => {
  // Both halves of availability - the user's setting and the binary on PATH -
  // must hold for a panel to be present. This is the predicate the activation
  // path reconciles against.
  const enabledAndAvailable = (
    descriptors: IProviderDescriptor[],
    settings: Record<string, boolean>,
    available: Record<string, boolean>
  ): string[] =>
    descriptors
      .filter(d => settings[d.id] !== false && available[d.id] === true)
      .map(d => d.id);

  const registry = new ProviderRegistry(PROVIDERS);

  // `Object.fromEntries` is ES2019 and this project targets ES2018.
  const allPresent = (): Record<string, boolean> => {
    const out: Record<string, boolean> = {};
    registry.ids.forEach(id => (out[id] = true));
    return out;
  };

  it('docks a provider only when the setting and the binary agree', () => {
    const all = allPresent();
    expect(enabledAndAvailable(registry.descriptors, {}, all)).toEqual(
      registry.ids
    );
    expect(
      enabledAndAvailable(registry.descriptors, { claude: false }, all)
    ).not.toContain('claude');
    expect(
      enabledAndAvailable(registry.descriptors, {}, { ...all, codex: false })
    ).not.toContain('codex');
  });

  it('treats a missing enable key as enabled', () => {
    const all = allPresent();
    // Only an explicit `false` disables, so a fresh install runs everything.
    expect(enabledAndAvailable(registry.descriptors, {}, all).length).toEqual(
      registry.ids.length
    );
  });

  it('leaves zero panels when nothing is available, without throwing', () => {
    expect(enabledAndAvailable(registry.descriptors, {}, {})).toEqual([]);
  });
});
